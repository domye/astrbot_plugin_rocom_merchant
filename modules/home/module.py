...
class HomeModule(BaseModule):
    ...
    async def _status(self, event: AstrMessageEvent, _=None):
        ...
        uid = sub.get("uid")
        polling = "轮询中" if sub.get("polling") else "等待成熟"
        timers = sub.get("timers", {})
        
        lines = [f"订阅UID: {uid}\n状态: {polling}"]
        if timers:
            lines.append(f"等待成熟: {len(timers)}株植物")
    ...
    async def _check_home(self, sub: Dict[str, Any]):
        ...
        for plant in plants:
            ...
            timer_key = str(plant_id)
            
            if rip_time <= now:
                await self._notify_ripe(session_id, user_id, plant, uid)
                await self.subscription.clear_timer(session_id, user_id, plant_id)
            elif timer_key not in timers:
                await self.subscription.set_timer(session_id, user_id, plant_id, rip_time)
                await self.subscription.set_polling(session_id, user_id, False)
                asyncio.create_task(self._wait_ripe(session_id, user_id, plant_id, rip_time, uid))
        ...
        try:
            ...
            for p in plants:
                if p.get("plant_id") == plant_id:
                    await self._notify_ripe(session_id, user_id, p, uid)
                    break
    ...
    async def _notify_ripe(self, session_id: str, user_id: str, plant: Dict[str, Any], uid: str):
        seed_id = plant.get("plant_seed_id", "未知")
        land_index = plant.get("land_index", 0)
        
        chain = [
            Comp.At(qq=str(user_id)),
            Comp.Plain(f" 你的家园({uid})作物已成熟!\n地块: {land_index + 1}号, 种子ID: {seed_id}")
        ]
